#!/bin/bash
#set -x
API_URL="http://localhost:8080/api/sensors/batch"
...
# Основной while
while true; do
	if [ '5' != $COUNT_VALUE ]; then
		IFS='|' read -r engine_temp tire_pressure fuel_level speed <<< "$(gen_data)"
